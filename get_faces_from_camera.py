...
path_photos_from_camera = "img/data_faces_from_camera/"
...
def pre_work_del_old_face_folders():
    ...
    if os.path.isfile("img/features_all.csv"):
        os.remove("img/features_all.csv")
...
if os.listdir("img/data_faces_from_camera/"):
    # 获取已录入的最后一个人脸序号 / get the num of latest person
    person_list = os.listdir("img/data_faces_from_camera/")
    person_num_list = []
    for person in person_list:
        person_num_list.append(int(person.split('_')[-1]))
        #例如person_1 person_2,将进行切片， 以下划线_分割，[-1],表示取切割后倒数第一个元素。该句将1，2，3分割出来并转成int，存入list
    person_cnt = max(person_num_list)
    #然后取最大，就是有图像的最后一个，从这个开始继续保存图像
...
while cap.isOpened():
    ...
    if len(faces) != 0:
        ...
        for k, d in enumerate(faces):
            #k,d  faces 是tuple[(),(),()], k取得是[]，d取的是[()()()],里面的（）
